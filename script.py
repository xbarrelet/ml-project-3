...
#     rmse_train_accuracy = np.sqrt(-1 * train_score)
#     rmse_test_accuracy = np.sqrt(-1 * test_score)
#
...
#     final_rmse2 = np.sqrt(final_mse)
#     print(f"RMSE2:{final_rmse2}")
#
    ...
    return [
        ...
        # },
        {
            **data_transformers_parameters_grid,
            ...
            # 'model__max_depth': [6], 'model__max_features': [7]
            'model': [GradientBoostingRegressor(n_estimators=300)],
...
def explain_results_using_lime(x_train, x_test, model_name, model, feat_names):
    ...
    for i in range(5):
        try:
            ...
            barplot.set(xlabel=None)
            barplot.set(ylabel=None)
            save_plot(barplot, f"lime_results_{model_name}_{i}", "lime_results")
...
def evaluate_impact_of_energy_star_score(model_parameters, model_name):
    ...
    save_plot(performance_plot, f"Star_score_performance_plot", "comparison")
